...
const Pizza = () => {
     ...
     const [counts, setCounts] = useState(Array(pizzas.length).fill(0));
      ...
      const dispatch= useDispatch()
        ...
        const [inCart, setInCart] = useState(Array(pizzas.length).fill(false));
    ...
    <div className="mx-auto max-w-[1300px]">
      ...
      <div className="flex items-center font-light shadow-xl px-4 py-2">
        ...
        <ul className="flex gap-10 ml-auto items-center mr-3 ">
          ...
          <Link to="/CartPage1"><li className="flex items-center gap-2 font-bold hover:text-orange-500 cursor-pointer">
      ...
      <p className="text-[10px] mt-10 ml-[280px] hover:text-orange-500">
        Home / Madurai / PIZZA HUT
      </p>

      {/* Restaurant Info */}
      <p className="font-bold text-3xl mt-[30px] ml-[280px]">Pizza Hut</p>
      ...
      {/* Food Items */}
      <p className="text-lg font-bold mt-10 ml-[200px]">Pizzas (20)</p>
      <div>
        {pizzas.map((pizza, index) => (
          ...
          >
            <p className="text-lg font-bold -mt-40">{pizza.name}</p>
            <p className="text-lg font-bold">{pizza.price}</p>

            <div className="flex items-center mt-2">
              <img className="w-[15px] h-[15px] " src={star1} alt="star" />
              <p className="text-green-900 font-bold ml-2">{pizza.rating}</p>
            </div>

            <p className="text-sm mt-2">{pizza.desc}</p>
            ...
            onClick={() => toggleCart(index, pizza)}
          >
